...
export const medicinePurchasesRouter = new Hono<AppContext>();
medicinePurchasesRouter.use("*", requireAuth);

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

type MedicinePurchase = {
  id: number;
  season_id: number;
...
medicinePurchasesRouter.get("/medicine-purchases", async (c) => {
  const seasonId = Number(c.req.query("season_id"));
  if (!seasonId) return c.json({ error: "season_id required" }, 400);
  const rows = await all<MedicinePurchase>(
    c.env.DB,
    "SELECT * FROM medicine_purchases WHERE season_id=? ORDER BY purchase_date DESC, id DESC",
    seasonId,
  );
  return c.json(rows);
});

medicinePurchasesRouter.post("/medicine-purchases", async (c) => {
  ...
  const result = await run(
    ...
    `INSERT INTO medicine_purchases
      (season_id, purchase_date, medicine_id, quantity, unit, unit_cost, total_cost, supplier, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    body.season_id, body.purchase_date, body.medicine_id,
    body.quantity, body.unit.trim(), body.unit_cost, body.total_cost,
    body.supplier?.trim() || null, body.notes?.trim() || null,
  );
  const row = await one<MedicinePurchase>(c.env.DB, "SELECT * FROM medicine_purchases WHERE id=?", result.meta.last_row_id);
  return c.json(row, 201);
});

medicinePurchasesRouter.patch("/medicine-purchases/:id", async (c) => {
  const id = Number(c.req.param("id"));
  const existing = await one<MedicinePurchase>(c.env.DB, "SELECT * FROM medicine_purchases WHERE id=?", id);
  ...
  await run(
    c.env.DB,
    `UPDATE medicine_purchases SET ${set}, updated_at=datetime('now') WHERE id=?`,
    ...Object.values(updates), id,
  );
  const row = await one<MedicinePurchase>(c.env.DB, "SELECT * FROM medicine_purchases WHERE id=?", id);
  return c.json(row);
});

medicinePurchasesRouter.delete("/medicine-purchases/:id", async (c) => {
  const id = Number(c.req.param("id"));
  await run(c.env.DB, "DELETE FROM medicine_purchases WHERE id=?", id);
